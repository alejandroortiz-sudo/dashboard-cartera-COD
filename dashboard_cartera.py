...
# --- CONFIGURACIÓN DE TU EXCEL EN LA NUBE ---
ENLACE_GOOGLE_DRIVE = "https://docs.google.com/spreadsheets/d/181Hitca413vDHuXuzY7Qk6i_Auuk5mIP/edit?usp=drive_link&ouid=112840324074960907762&rtpof=true&sd=true"
...
# EDUCACIÓN: Detección inteligente del enlace
def obtener_enlace_directo(url):
    if "spreadsheets/d/" in url:
        try:
            file_id = url.split("/d/")[1].split("/")[0]
            # Link de exportación exclusivo para Google Sheets
        ...
        try:
            file_id = url.split("/d/")[1].split("/")[0]
            # Link de descarga cruda para Google Drive
...
# EDUCACIÓN: Rutina de descarga profesional a prueba de advertencias
def descargar_excel_drive(url_original):
    url_directa = obtener_enlace_directo(url_original)
    session = requests.Session()
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}

    response = session.get(url_directa, headers=headers, stream=True)

    # Verificamos si hay cookies de advertencia por archivo grande
    token = None
    ...
    # Si detecta la advertencia, forzamos la confirmación
    if token:
        if "?" in url_directa:
            url_confirmacion = f"{url_directa}&confirm={token}"
        else:
            url_confirmacion = f"{url_directa}?confirm={token}"
        response = session.get(url_confirmacion, headers=headers, stream=True)
...
st.markdown(
    f"<p style='text-align: center; color: gray; font-size: 14px; margin-top: -10px;'>Conectado a la nube. Última lectura: <b>{fecha_actualizacion}</b></p>",
    unsafe_allow_html=True)


@st.cache_data(ttl=7200)
def cargar_datos_desde_drive(url_drive):
    archivo_en_memoria = descargar_excel_drive(url_drive)

    diccionario_hojas = pd.read_excel(archivo_en_memoria, sheet_name=None, engine='openpyxl')
...
# --- HERRAMIENTAS DE SISTEMA (MENÚ LATERAL) ---
st.sidebar.markdown("### ⚙️ Actualización de Datos")
st.sidebar.write("El dashboard lee la base de datos automáticamente desde Google Drive.")

if st.sidebar.button("🔄 Forzar lectura de Google Drive", width="stretch"):
    st.cache_data.clear()
    st.rerun()

st.sidebar.divider()

if ENLACE_GOOGLE_DRIVE == "AQUI_PEGA_TU_ENLACE":
    st.error("⚠️ Falta configurar el enlace. Por favor, pega el enlace de tu Google Drive en el código (línea 11).")
    st.stop()

con_datos = st.spinner('Conectando a Google Drive y descargando información...')
try:
    df_principal, col_orden, col_item, col_fecha, col_guia, col_valor, col_marca, col_ciudad, col_transp, col_tipo, col_recaudo, col_rango, df_comisiones = cargar_datos_desde_drive(
        ENLACE_GOOGLE_DRIVE)
except Exception as e:
    st.error(f"🚨 Error al conectar con Google Drive. Detalle técnico: {e}")
    st.stop()
